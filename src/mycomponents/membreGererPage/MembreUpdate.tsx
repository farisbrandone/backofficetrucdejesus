...
function MembreUpdate() {
  ...
  const { membreId } = useParams<string>();
  ...
  const UpdateMembre = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast({
          title: "Success",
          description: "La mise à jour s'est effectuée avec success",
        });
        setStartSending(() => false);
      ...
      toast({
        ...
        title: "Erreur",
        description:
          "Une erreur est survenue pendant l'envoie de la mise à jour, vérifier votre connexion",
      });
      setStartSending(() => false);
      console.error("");
    }
  ...
  useEffect(() => {
    async function getEventDataForUpdateWithId() {
      try {
        ...
        setImage(data.image);
        setSexe(data.sexe);
        setDateOfBirth(new Date(data.birthDay));
