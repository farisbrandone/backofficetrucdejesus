...
function Terms() {
  ...
  const [alreadyExist, setAlreadyExist] = useState<TermsData>();
  ...
  const sendTerms = async () => {
    ...
    if (alreadyExist) {
      try {
        const result = await requestToUpdateUniversalDataWithId<TermsData>(
    ...
    const resultAll = await requestToSetUniversalData<TermsData>(
      "TermsData",
      data
    );
    ...
    const getAllData = async () => {
      try {
        ...
        if (result.length > 0) {
          ...
          setTermsText(result[0].termsText);
          return;
        }
      } catch (error) {
        setLoadingFail(true);
  ...
  return (
    <TabsContent value="terms">
      <Card className="mt-5">
        <CardContent className="space-y-2 text-[16px] sm:text-[18px] flex flex-col gap-3 max-w-[1250px] ">
          ...
          <div className="flex flex-col gap-2 items-center ">
            <ComplexeDescription
              ...
              onBlurValue={(newContent) => setTermsText(newContent)}
            />
          </div>
        </CardContent>
        {startSending && (
        ...
        <CardFooter>
          <Button disabled={startSending} onClick={sendTerms}>
            Enregistrer
          </Button>
        </CardFooter>
      </Card>
