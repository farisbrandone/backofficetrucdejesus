...
function UpdatePopupBanners() {
  ...
  const { bannerId } = useParams<string>();
  ...
  useEffect(() => {
    async function getData() {
      try {
        const data =
          await requestToGetAllUniversalDataWithId<PopupBannersDataType>(
            bannerId as string,
            "PopupBannersData"
          );
        setTitle(data.title);
        setTargetUrl(data.targetUrl);
        setStatusCommunityPage(data.statusCommunityPage);
        setStatusGroupePage(data.statusGroupePage);
        setGroupePageAssociate(data.groupePageAssociate);
        setImage(data.image);
        setTypes(data.types);
  ...
  if ((!title || !targetUrl) && !loadingFail) {
    return (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-[12px] sm:text-[18px]">
            Mis à jour du Popup Banners
          </CardTitle>
          <CardDescription>Modifier les champs suivants</CardDescription>
        </CardHeader>
        <CardContent className="space-y-2 flex-col gap-3 text-[16px] sm:text-[18px]">
          ...
          <div className="flex items-center gap-2">
            ...
            <Label className="flex items-center gap-1">
              <p className="-mr-3">Community Page</p>
          ...
          {loadingGroupeData ? (
            ...
            <div className="space-y-1 w-[350px] h-[250px] rounded-xl shadow-xl bg-[#eeeded] text-[#191919] overflow-y-scroll flex flex-col items-center gap-3 ">
          ...
          <div className="space-y-1">
            <Label htmlFor="typePopup">Type de Popup</Label>
            <Select value={types} onValueChange={(value) => handleTypes(value)}>
              <SelectTrigger className="sm:w-[600px] text-[16px] sm:text-[18px]">
                <SelectValue placeholder="Choisir..." />
              </SelectTrigger>
              <SelectContent className="w-full border-[#191919] border-[1px] border-solid">
                <SelectItem value="popup de chargement">
                  Popup de chargement
                </SelectItem>
                <SelectItem value="popup retardé">Popup retardé</SelectItem>
                <SelectItem value="popup d'intention de sortie">
...
export default UpdatePopupBanners;
